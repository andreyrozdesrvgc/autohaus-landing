/*
 * ⚠️  ВНИМАНИЕ! ЭТОТ ФАЙЛ БОЛЬШЕ НЕ ЯВЛЯЕТСЯ ИСТОЧНИКОМ КОНТЕНТА!
 *
 * Все тексты, картинки и видео сайта хранятся в MongoDB и редактируются
 * через админ-панель /admin. Правки в этом файле НЕ отобразятся на сайте.
 *
 * КАК ПОМЕНЯТЬ КАРТИНКУ/ВИДЕО:
 *   1. Открыть /admin (логин из backend/.env)
 *   2. Выбрать вкладку с нужной секцией
 *   3. В поле медиа нажать "Загрузить файл" или "Импортировать по ссылке"
 *   4. Нажать "сохранить"
 *
 * Этот файл используется ТОЛЬКО как fallback: если API недоступен, сайт
 * покажет эти URL вместо пустого экрана. Обычные пользователи его никогда не увидят.
 * ─────────────────────────────────────────────────────────────────────
 *
 * ============================================================================
